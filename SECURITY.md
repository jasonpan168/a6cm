...
**唯一的漏洞报告渠道是 GitHub 私密安全通报（Security Advisory）：**

👉 **<https://github.com/jasonpan168/a6cm/security/advisories/new>**

（也可以从仓库首页进入 **Security → Advisories → Report a vulnerability**。）

该通道只有你和维护者可见，在通报公开发布之前不会泄露任何细节。

**请不要：**

- ❌ 在公开的 Issue、Pull Request、Discussions 或社交媒体中提交或讨论漏洞细节；
- ❌ 通过邮件报告 —— 本项目不提供也不接受邮件漏洞报告渠道。

**报告时请尽量包含：**

1. 受影响的文件与版本（commit 号最佳）
2. 漏洞类型与影响范围（可读数据？可写数据？可 RCE？需要登录吗？）
3. 可复现的最小步骤或 PoC
4. 你认为合适的修复建议（可选）
...
| 初步评估与定级 | 10 个工作日内 |
| 发布修复 | 视严重程度而定，高危优先 |
| 公开披露 | 修复发布后，通过同一条 Advisory 公开 |

我们遵循协调披露（coordinated disclosure）：请在修复发布前不要公开细节。若你愿意，修复公开时会在 Advisory 中署名致谢。
